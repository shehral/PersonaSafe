...
def mock_model_and_tokenizer():
    ...
    mock_hidden_states = [torch.randn(1, 5, 1024) for _ in range(12)] # 12 layers of hidden states
...
def test_persona_extractor_logic(mock_tokenizer_from_pretrained, mock_model_from_pretrained, mock_model_and_tokenizer):
...
@patch('transformers.AutoModelForCausalLM.from_pretrained')
@patch('transformers.AutoTokenizer.from_pretrained')
def test_extractor_uses_cache(mock_tokenizer_loader, mock_model_loader, mock_cache_class):
    ...
    extractor.extract_activations = MagicMock(return_value=torch.randn(1024))
    ...
    mock_cache_instance.set.assert_called_once_with("mock-model-cache-test", "caching_trait", vec1)
